...
class PConv_Unet_Swapping(nn.Module):
    ...
    def forward(self,input_x,mask):
		...
		# 512 x 18 x 18
        out_key = "h_{:d}".format(3)
        encoder_out, out_mask = encoder_dict[out_key], mask_dict[out_key]

        ###################    patch swappping module  ###################
        threshold = 5.5/9
        h_gaussian = 5
        reshaped_encoder_out = torch.Tensor( encoder_out.detach().cpu().view((-1,512,18,18)).numpy().transpose(0,2,3,1) ).cuda()
        ...
        ###################   last encoder layer  ###################
        encoder_key = "encoder_{:d}".format(4)
        key = "h_{:d}".format(4)
        out_data, out_mask = getattr(self, encoder_key)(out_feature_map,out_mask)
