...
def conv(in_f, out_f, kernel_size, stride=1, pad='zero'):
    ...
    convolver = spectral_norm(nn.Conv2d(in_f, out_f, kernel_size, stride, padding=to_pad, bias=False))
...
def decodernw_SN(
    ...
    for i in range(len(num_channels_up)-1):
        ...
        if i != len(num_channels_up)-1:	
            if(bn_before_act): 
                model.add(spectral_norm(nn.BatchNorm2d( num_channels_up[i+1],affine=bn_affine)))
            model.add(act_fun)
